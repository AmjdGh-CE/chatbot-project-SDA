...
async def load_chat(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        db.close()
        logging.error(e)
        return func.HttpResponse(body=json.dumps({"detail": f"An error occurred: {str(e)}"}), status_code=500)
    ...
    try:
        ...
        with db.cursor() as cursor:
            cursor.execute(
                ...
                """,
                (chat_id, req.get_json()["chat_name"], file_path, req.get_json()["pdf_path"], req.get_json()["pdf_name"], req.get_json()["pdf_uuid"]),
            )
        db.commit()
        db.close()
